document.addEventListener("DOMContentLoaded", function() {
    ...
    // Elementos básicos protegidos da estrutura do site
    const navbar = document.querySelector("#navbar");
    const counterElement = document.getElementById("live-chat-count");

    // 1. Sistema do Menu Superior (Navbar Scroll)
    if (navbar) {
    ...
    // 2. Sistema de Revelação de Conteúdo (Scroll Reveal)
    try {
        ...
        console.log("Observer não suportado neste aparelho.");
    }

    // 3. Sistema do Contador Técnico de Atendimentos
    let activeClients = Math.floor(Math.random() * (19 - 11 + 1)) + 11;
    if (counterElement) {
        counterElement.textContent = activeClients;
    ...
    // 4. BLOCO ENCAPSULADO DA IMPRESSORA 3D (Se der erro aqui, o resto do site continua funcionando)
    try {
        ...
        if (axisX && nozzleHead && printerFrame && pathP && pathH && path3 && pathD) {
            ...
            // Função de conversão viewBox para coordenadas reais na tela
            ...
            // Banco de dados cronológico de movimentação
            let timeline = [
                // Letra P
        ...
        console.error("Erro na animação da impressora 3D: ", err);
    }
});
